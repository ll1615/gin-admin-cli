...
func getModelFileName(dir, name string) string {
	fullname := fmt.Sprintf("%s/app/model/m_%s.go", dir, util.ToLowerUnderlinedNamer(name))
	return fullname
}
...
type I{{.Name}} interface {
	...
	// 查询指定数据
	Get(ctx context.Context, id int, opts ...schema.{{.Name}}GetOptions) (*schema.{{.Name}}, error)
	// 创建数据
	Create(ctx context.Context, item *schema.{{.Name}}) error
	// 更新数据
	Update(ctx context.Context, id int, item *schema.{{.Name}}) error
	// 删除数据
